...
class ParentNode extends Node {
  ...
  createStaticChild (path) {
    ...
    if (staticChild) {
      let i = 1
      const staticChildPrefix = staticChild.prefix
      const staticChildPrefixLength = staticChildPrefix.length
      for (; i < staticChildPrefixLength; i++) {
        if (path.charCodeAt(i) !== staticChildPrefix.charCodeAt(i)) {
...
class StaticNode extends ParentNode {
  ...
  getParametricChild (regex) {
    const regexpSource = regex?.source

    const parametricChild = this.parametricChildren.find(child => {
      const childRegexSource = child.regex?.source
