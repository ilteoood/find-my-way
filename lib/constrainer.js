...
class Constrainer {
  ...
  hasConstraintStrategy (strategyName) {
    ...
    return customConstraintStrategy !== undefined && (
  ...
  addConstraintStrategy (strategy) {
    ...
    assert(typeof strategyName === 'string' && strategyName !== '', 'strategy.name is required.')
    assert(strategy.storage && typeof strategy.storage === 'function', 'strategy.storage function is required.')
    assert(strategy.deriveConstraint && typeof strategy.deriveConstraint === 'function', 'strategy.deriveConstraint function is required.')

    if (this.strategies[strategyName]?.isCustom) {
      throw new Error(`There already exists a custom constraint with the name ${strategyName}.`)
    }

    if (this.isStrategyUsed(strategyName)) {
      throw new Error(`There already exists a route with ${strategyName} constraint.`)
    }
    ...
    if (strategy.mustMatchWhenDerived) {
      this.noteUsage({ [strategyName]: strategy })
    }
  }
